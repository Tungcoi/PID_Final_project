...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  // -------------------------
  // In the FOLLOW_LANE state, check if we are approaching an intersection.
  // If so, transition to DECEL_TO_STOP to prepare for stopping.
  // -------------------------
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    // distance
    goal = _goal;  // Giữ nguyên mục tiêu dừng
    ...
    goal = _goal;  // Giữ nguyên mục tiêu tại điểm dừng

    // -------------------------
    // Calculate how long the car has been stopped.
    // If the car has stopped long enough and the traffic light is not "Red", transition back to FOLLOW_LANE.
    // If the light is "Yellow", reduce the speed.
    // -------------------------
