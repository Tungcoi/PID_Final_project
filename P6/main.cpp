...
int main ()
{
  ...
  **/
  ...
  pid_steer.Init(0.3, 0.0025, 0.17, 0.60, -0.60);    // Khởi tạo bộ điều khiển góc lái (steering) với các tham số PID
  pid_throttle.Init(0.21, 0.0006, 0.080, 1.0, -1.0);  // Khởi tạo bộ điều khiển tốc độ (throttle) với các tham số PID
...
//           pid_steer.UpdateDeltaTime(new_delta_time);
          ...
          double error_steer = angle_between_points(x_position, y_position, x_points.back(), y_points.back()) - yaw;



          //double steer_output;
          double steer_output = pid_steer.TotalError();  // Lấy giá trị đầu ra điều khiển góc lái từ PID
...
//           double throttle = pid_throttle.TotalError();
          double throttle_response = pid_throttle.TotalError();  // Lấy giá trị đầu ra điều khiển tốc độ từ PID
